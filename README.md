...
- Image generation defaults to `OPENAI_IMAGE_SIZE=832x832`, `OPENAI_IMAGE_OUTPUT_FORMAT=jpeg`, and `low` quality for both challenge and student images. Size and quality are the two speed and cost levers. `gpt-image-2` takes any `WIDTHxHEIGHT` whose edges both divide by 16 within a 1:3–3:1 aspect ratio, but it also enforces an undocumented minimum pixel budget: measured against the live API, `720x720` and `768x768` are refused, `832x832` renders at ~103KB. So `832x832` is the smallest square available — 34% fewer pixels than `1024x1024`. Older models (`gpt-image-1`, `gpt-image-1-mini`) take only `1024x1024`, `1536x1024`, `1024x1536` or `auto`, and fall back to `1024x1024` automatically. An unusable value never reaches the API — it falls back rather than failing mid-lesson. Raise size or quality if the dragons look too rough.

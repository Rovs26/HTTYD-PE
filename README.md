...
- Image generation defaults to `OPENAI_IMAGE_SIZE=1024x1024`, `OPENAI_IMAGE_OUTPUT_FORMAT=jpeg`, and `low` quality for both challenge and student images. Quality is the speed and cost lever; the size stays at 1024x1024. `gpt-image-2` will accept a smaller `WIDTHxHEIGHT` (edges divisible by 16, aspect 1:3–3:1, above an undocumented minimum pixel budget — measured: 720x720 and 768x768 are refused, 832x832 renders), so a smaller size can be set deliberately, but nothing ships below 1024x1024. An unusable value falls back rather than failing mid-lesson. Raise quality if the dragons look too rough.
